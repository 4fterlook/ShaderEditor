...
#if NOISE_METHOD == 0
  x = p + f;
  return texture2D(noise, (x + 0.5) / 32.0, 0.0).x * 2.0 - 1.0;
#endif
#if NOISE_METHOD == 1
...
vec4 raymarch(in vec3 ro, in vec3 rd, in vec3 bgcol, in ivec2 px) {
  ...
  float t = tmin + 0.1 * texelFetch(noise, px & 1023, 0).x;
...
// lod
    ...
    if (den > 0.01) // if inside
    {
      ...
      vec4 col =
          vec4(mix(vec3(1.0, 0.93, 0.84), vec3(0.25, 0.3, 0.4), den), den);
      col.xyz *= lin;
      // fog
...
#define MARCH(STEPS, MAPLOD)                                                   \
  for (int i = 0; i < STEPS; i++) {                                            \
    ...
    if (den > 0.01) {                                                          \
      ...
      vec4 col =                                                               \
          vec4(mix(vec3(1.0, 0.95, 0.8), vec3(0.25, 0.3, 0.35), den), den);    \
      col.xyz *= lin;                                                          \
...
vec4 raymarch(in vec3 ro, in vec3 rd, in vec3 bgcol, in ivec2 px) {
  vec4 sum = vec4(0.0);
  ivec2 tmp = px & 255;
  float t = 0.05 * texelFetch(noise, px & 255, 0).x;
  MARCH(40, map5);
  MARCH(40, map4);
  MARCH(30, map3);
  MARCH(30, map2);
...
vec4 render(in vec3 ro, in vec3 rd, in ivec2 px) {
  ...
  float sun = clamp(dot(sundir, rd), 0.0, 1.0);
  vec3 col =
      vec3(0.6, 0.71, 0.75) - rd.y * 0.2 * vec3(1.0, 0.5, 1.0) + 0.15 * 0.5;

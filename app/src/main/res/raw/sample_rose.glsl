...
mat3 rotationXY(vec2 angle) {
  ...
  return mat3(c.y, 0.0, -s.y, s.y * s.x, c.x, c.y * s.x, s.y * c.x, -s.x,
              c.y * c.x);
}
...
vec3 gradient(vec3 pos) {
  ...
  return normalize(vec3(map(pos + dx) - map(pos - dx),
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (pointers[0].x > 0.0)
    rot = rotationXY(pointers[0].yx / resolution.yx * vec2(PI, -2.0 * PI) +
                     vec2(PI * -0.5, PI));
  ...
  float r = 1.2 - length((fragCoord.xy / resolution.xy) - 0.5) * 1.0;
